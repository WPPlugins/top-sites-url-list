...
function reg_tsul_admin_scripts_and_styles() {
	...
	wp_register_script( 'tsul-admin-scripts', plugins_url( 'js/tsul_admin_script.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), '1.0.0', true );

    wp_enqueue_style('jquery-ui-style',  plugins_url( 'css/jquery-ui.css', __FILE__ ), false, '1.10.4');
	wp_enqueue_style('tsul-timepicker-styles', plugins_url( 'css/jquery-ui-timepicker-addon.min.css', __FILE__ ), array( 'jquery-ui-style' ), '1.4.3');

	wp_enqueue_script('tsul-timepicker-script', plugins_url( 'js/jquery-ui-timepicker-addon.min.js', __FILE__ ),  array('jquery-ui-core', 'jquery-ui-datepicker' ), '1.4.3', true);
}
...
function cron_add_recurrance ( $schedules ) {
	...
	$schedules['tsul_cron_recurrance'] = array(
        ...
        foreach ($widget_tsul as $widget_id => $widget) {
            ...
            if ( isset($results->rows) && is_array($results->rows) ) {
                ...
                foreach ($results->rows as $row) {
                    ...
                    if ( isset($clean_url[1]) || ((!isset($widget['tsul_exclude_hp']) || $widget['tsul_exclude_hp'] != 1) && $row[0] == '/' ) ) {
                        ...
                        if ( $post_result && $hp_page_exist && ((is_array($used_post_types) && count($used_post_types) > 0 && in_array($post_result->post_type, $used_post_types)) || (is_array($used_post_types) && count($used_post_types) == 0) )) {
                            ...
                            $post_save['tsul-'.$widget_id][] = array(
                                'post_id' 			=> $post_result->ID,
                                'post_title' 		=> $post_title,
                                'post_permalink' 	=> $clean_url[1],
                                'post_views' 		=> $row[1],
                            );
...
if ( get_option('tsul_full_stats') != '' && get_option('tsul_show_in_admin_tables') == 1 ) {
    ...
    if ( is_array($used_post_types) ) {
        ...
        foreach ($used_post_types as $post_type) {
            switch ( $post_type ) {
                case 'post': $post_type = 'posts';
                    break;
                case 'page': $post_type = 'pages';
                    break;
            }
